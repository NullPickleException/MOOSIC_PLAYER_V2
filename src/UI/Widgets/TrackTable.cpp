...
void TrackTable::DrawRows(const std::vector<const MusicTrack*>& tracks)
{
    ...
    for (size_t i = 0; i < tracks.size(); ++i)
    {
        ...
        for (size_t colIdx = 0; colIdx < m_config.Columns.size(); ++colIdx)
        {
            ...
            switch (column)
            {
            case TrackColumn::Title:     fullText = track->GetTitle(); break;
            case TrackColumn::Artist:    fullText = track->GetArtist(); break;
            case TrackColumn::Album:     fullText = track->GetAlbum(); break;
            case TrackColumn::Extension:
            {
            ...
            if (column == TrackColumn::Title)
            {
                ...
                if (ImGui::IsItemHovered())
                {
                    ...
                    if (m_onRowHover) m_onRowHover(event);
                ...
                if (clicked)
                {
                    ...
                    RowEventData event = CreateRowEvent(rowIndex, track);
                    if (m_onRowClick) m_onRowClick(event);
                }

                if (ImGui::IsMouseDoubleClicked(0) && ImGui::IsItemHovered())
                {
                    RowEventData event = CreateRowEvent(rowIndex, track);
                    if (m_onRowDoubleClick) m_onRowDoubleClick(event);
                }
                ...
                if (ImGui::IsItemHovered() && ImGui::IsMouseClicked(ImGuiMouseButton_Right))
                {
                    ...
                    RowEventData event = CreateRowEvent(rowIndex, track);
                    if (m_onRowRightClick) m_onRowRightClick(event);
                }
            }
            else
            {
        ...
        ImVec4 rowColor;
        if (isPlaying)       rowColor = m_style.RowPlaying;
        else if (isSelected) rowColor = m_style.RowSelected;
        else if (isHovered)  rowColor = m_style.RowHovered;
        else                 rowColor = (rowIndex % 2 == 0) ? m_style.RowDefault : m_style.RowDefaultAlt;
...
float TrackTable::GetColumnWidth(TrackColumn column) const
{
    switch (column)
    {
    case TrackColumn::Title:     return m_style.TitleWidth;
    case TrackColumn::Artist:    return m_style.ArtistWidth;
    case TrackColumn::Album:     return m_style.AlbumWidth;
    case TrackColumn::Extension: return m_style.ExtensionWidth;
    case TrackColumn::Duration:  return (m_style.DurationWidth < 70.0f) ? 70.0f : m_style.DurationWidth;
    default:                     return m_style.DefaultColumnWidth;
    }
}
