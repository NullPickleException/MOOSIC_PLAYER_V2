...
{
    // Apply table config that lives in the data model
    ...
    // Table callbacks – only forward to the model
    ...
    m_trackTable.OnRowRightClick([this](const RowEventData& event)
        ...
        m_contextMenu.Open(ImGui::GetMousePos().x, ImGui::GetMousePos().y);
    });

    m_trackTable.OnSort([this](const SortRequest& sort)
                        { m_data.ApplySort(sort); });

    // Only write config when the user actually changes columns
    m_trackTable.OnColumnsChanged([this](const TrackTableConfig& config)
                                  { m_data.SetTableConfig(config); });

    // Edit dialog → model mutation API (no const_cast)
    m_editTrackDialog.SetSaveCallback(
        [this](const MusicTrack* track,
               const std::string& title,
               const std::string& artist,
               const std::string& album)
        {
            if (track)
                m_data.UpdateTrackMetadata(track->GetId(), title, artist, album);
        });

    // Theme for edit dialog (will be overwritten by ApplyTheme later if needed)
    {
...
{
    // Keep table config in sync (read-only from model)
    m_trackTable.ApplyConfig(m_data.GetTableConfig());
...
{
    const bool anyToolbar = m_toolbarOptions.ShowSearchBar ||
    ...
    if (m_toolbarOptions.ShowRefreshButton)
    {
        ...
        {
            // Pure data-layer refresh. Scanning for new files is owned higher up
            // (UI / DirectoryDataModel) if desired.
            m_data.Refresh();
        }
...
void LibraryWindow::BuildContextMenu(std::vector<MenuItem>& items)
{
    ...
    if (m_playlistModel && !m_playlistModel->GetAllPlaylists().empty())
    {
        ...
        {
            playlistItems.push_back({playlists[j].name, true, false, [this, j]()
                                     {
...
void LibraryWindow::SetupSearchBar()
{
    ...
    m_searchBar.SetSearchCallback(
        [this](const std::string& query) -> std::vector<TrackSearchResult>
        {
            // Clear any active filter so we search the full set
            m_data.SetSearchFilter("");
    ...
    m_searchBar.SetSelectCallback(
        [this](const TrackSearchResult& result)
        {
